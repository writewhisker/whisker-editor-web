...
import { DeadLinksValidator } from '@writewhisker/core-ts';
...
describe('DeadLinksValidator', () => {
  ...
  it('should detect choice pointing to non-existent passage', () => {
    ...
    expect(issues[0].message).toContain('non-existent passage');
  ...
  it('should include passage and choice information in issue', () => {
    ...
    expect(issues[0].description).toContain('non-existent passage');
  });
