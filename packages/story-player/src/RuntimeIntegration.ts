/**
 * WLS Integration Module
 *
 * Unified facade for all WLS story player components.
...
/**
 * WLS integrated player configuration
 */
 ...
 */
export type RuntimeEventCallback = (event: RuntimeEvent, data: unknown) => void;

/**
 * RuntimeIntegration - Unified WLS story player
 *
 * Provides a single interface for all WLS features:
...
export class RuntimeIntegration {
   ...
   */
  off(event: RuntimeEvent, callback: RuntimeEventCallback): void {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
   ...
   */
  once(event: RuntimeEvent, callback: RuntimeEventCallback): void {
    const wrapper: RuntimeEventCallback = (e, data) => {
...
/**
 * Create a new WLS integrated player
 */
export function createPlayer(config?: RuntimeConfig): RuntimeIntegration {
  return new RuntimeIntegration(config);
}

/**
 * Create a minimal WLS player for testing
 */
export function createTestPlayer(): RuntimeIntegration {
