/**
 * WLS 2.0 Container - Dependency Injection Container
 *
 * Manages all WLS 2.0 runtime components as a unified container.
 * Provides lifecycle management, state serialization, and coordinated updates.
 *
 * Reference: whisker-core/lib/whisker/wls2/init.lua
 */
...
/**
 * WLS2Container - Unified container for all WLS 2.0 runtime components
 *
 * @example
 * ```typescript
...
export class WLS2Container {
  ...
  private options: Required<WLS2Options>;
  private initialized: boolean = false;

  constructor(options: WLS2Options = {}) {
    this.options = {
  ...
  get threads(): ThreadScheduler {
    if (!this._threads) {
      throw new Error('WLS2Container: Threads are disabled');
    }
  ...
  get timers(): TimedContent {
    if (!this._timers) {
      throw new Error('WLS2Container: Timers are disabled');
    }
    return this._timers;
  }
  ...
  get externals(): ExternalFunctions {
    if (!this._externals) {
      throw new Error('WLS2Container: External functions are disabled');
    }
  ...
  get lists(): ListRegistry {
    if (!this._lists) {
      throw new Error('WLS2Container: Lists are disabled');
    }
    return this._lists;
  }
  ...
  get textEffects(): TextEffects {
    if (!this._textEffects) {
      throw new Error('WLS2Container: Text effects are disabled');
    }
  ...
  get audioEffects(): AudioEffects {
    if (!this._audioEffects) {
      throw new Error('WLS2Container: Audio effects are disabled');
    }
  ...
  get passages(): ParameterizedPassages {
    if (!this._passages) {
      throw new Error('WLS2Container: Parameterized passages are disabled');
    }
   ...
   */
  deserialize(state: WLS2State): void {
   ...
   * Get current options
   */
  getOptions(): Required<WLS2Options> {
    return { ...this.options };
  }
...
/**
 * Factory function to create a WLS2Container
 */
export function createWLS2Container(options?: WLS2Options): WLS2Container {
  return new WLS2Container(options);
}

/**
 * Create a minimal container for testing (all features enabled)
 */
export function createTestContainer(): WLS2Container {
  return new WLS2Container({
    enableThreads: true,
    enableTimers: true,
 ...
 */
export function createLightContainer(): WLS2Container {
  return new WLS2Container({
    enableThreads: true,
    enableTimers: true,
    enableExternalFunctions: true,
